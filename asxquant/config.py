...
# v7 (第二轮外部 review 的批次一，纯 bug 修复):
#     · 删掉 rev_ret5 —— 它与 mom_5 逐位相同（相关 1.0000）却先验符号相反，
#       让 combo 双倍投票、L2 对该变量变相放松、block 归因正负抵消。46 -> 45。
#     · picks/validate 的评分加单位方差归一化（覆盖率不同会让分数方差不同，
#       缺成分的股票被系统性推向排序两端，实测超配 15.7%）。
#     · 回测执行时点后移 1 日（评分用 t 日收盘数据，成交只能在 t+1）。
#     · evaluate 的 accuracy 阈值改用基准率。
#     ⚠️ 建模层（拟合/校准/集成）本批仍未改动 —— combo 的块内平均、近因加权、
#        专家聚合等属于第二步，刻意不混进来。
MODEL_VERSION = "v7"
